...
class CreateDataUserTestCase(SetupData):
    def test_create_data_siswa_user(self):
        ...
        response = self.client.post(
            reverse("import_data_siswa_user"), data, format="multipart"
        )
    ...
    def test_create_data_guru_user(self):
        ...
        data = {"file": uploaded_file}
        response = self.client.post(
            reverse("import_data_guru_user"), data, format="multipart"
        )
    ...
    def test_create_data_karyawan_user(self):
        # create data karyawan
        data_guru_response = self.client.post(
            reverse("data_karyawan"), self.data_karyawan
        )
        ...
        data = {"file": uploaded_file}
        response = self.client.post(
            reverse("import_data_karyawan_user"), data, format="multipart"
        )
