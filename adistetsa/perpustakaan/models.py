...
class Pendanaan(models.Model):
    ...
    NAMA_PENDANAAN = models.CharField(
        max_length=255, validators=[cek_huruf_besar_awal_kalimat]
    )
...
class KatalogBuku(models.Model):
    REGISTER = models.CharField(
        primary_key=True, max_length=255, validators=[validasi_integer]
    )
    ...
    KOTA_PENERBIT = models.CharField(
        max_length=255, validators=[cek_huruf_besar_awal_kalimat]
    )
...
class KatalogBukuCopy(models.Model):
    ...
    def __str__(self):
        return (
            str(self.REGISTER_COPY)
            + " - "
            + str(self.DATA_DONASI.REGISTER_DONASI.JUDUL)
        )


def post_save_donasi_buku(sender, instance, **kwargs):
    try:
        donasi_buku = DonasiBuku.objects.filter(
            REGISTER_DONASI=instance.REGISTER_DONASI
        )
        jumlah_buku = []
            ...
            for i in range(data["jumlah"]):
                if data["data_donasi"] == instance.id:
                    buku_copy = KatalogBukuCopy.objects.update_or_create(
                        ...
                        REGISTER_COPY=str(instance.REGISTER_DONASI_id)
                        + str(last_index),
                    )
                last_index += 1
...
class PengajuanPeminjamanSiswa(models.Model):
    ...
    FILE_TTD_PENGAJUAN = models.FileField(
        max_length=255, upload_to="Dokumen_Peminjaman_Jangka_Panjang_Siswa", blank=True
    )
    ...
    if instance.STATUS_PENGAJUAN == "Disetujui":
        try:
            for data in instance.BUKU.values():
                ...
                if (
                    instance.JANGKA_PEMINJAMAN == "Jangka Panjang"
                    and instance.FILE_TTD_PENGAJUAN
                ):
                    obj.FILE_TTD_PENGAJUAN = duplikat_file(
                        instance,
                        instance.FILE_TTD_PENGAJUAN.read(),
                        instance.FILE_TTD_PENGAJUAN.name,
                    )
...
class PengajuanPeminjamanGuru(models.Model):
    ...
    FILE_TTD_PENGAJUAN = models.FileField(
        max_length=255, upload_to="Dokumen_Peminjaman_Jangka_Panjang_Guru", blank=True
    )
...
class RiwayatPeminjamanGuru(models.Model):
    ...
    FILE_TTD_PENGAJUAN = models.FileField(
        max_length=255, upload_to="Dokumen_Peminjaman_Jangka_Panjang_Guru", null=True
    )
    STATUS_PEMINJAMAN = models.CharField(
        max_length=255, choices=ENUM_STATUS_PEMINJAMAN, default="Sedang Dipinjam"
    )
