...
from .serializers import (
    DataSiswaUser,
    DataGuruUser,
    KatalogBukuListSerializer,
    KatalogBukuCopySerializer,
    PengajuanPeminjamanSiswaSerializer,
    PengajuanPeminjamanSiswaAdminSerializer,
    PengajuanPeminjamanSiswaListSerializer,
    PengajuanPeminjamanGuruSerializer,
    PengajuanPeminjamanGuruAdminSerializer,
    PengajuanPeminjamanGuruListSerializer,
    RiwayatPeminjamanGuruSerializer,
    RiwayatPeminjamanSiswaSerializer,
    RiwayatPeminjamanGuruAdminSerializer,
    RiwayatPeminjamanSiswaAdminSerializer,
)
...
class KatalogBukuListView(generics.ListAPIView):
    ...
    search_fields = (
        "JUDUL",
        "KODE_AUTHOR__NAMA_AUTHOR",
        "BAHASA__BAHASA",
        "KODE_TIPE__NAMA_TIPE",
...
class PengajuanPeminjamanSiswaListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if is_in_group(current_user, "Siswa"):
            ...
            queryset = PengajuanPeminjamanSiswa.objects.filter(
    ...
    def create(self, request, *args, **kwargs):
        ...
        if check_buku_tersedia(buku):
            ...
            return Response(
                data={"error": "Buku yang dipilih tidak tersedia untuk dipinjam."},
...
class PengajuanPeminjamanGuruListView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if is_in_group(current_user, "Guru"):
            ...
            queryset = PengajuanPeminjamanGuru.objects.filter(
                DATA_GURU=data_guru_user.DATA_GURU
            ).order_by("-TANGGAL_PENGAJUAN")
            return queryset
...
class RiwayatPeminjamanSiswaListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if is_in_group(current_user, "Siswa"):
            data_siswa_user = DataSiswaUser.objects.get(USER=current_user)
            queryset = RiwayatPeminjamanSiswa.objects.filter(
                NIS=data_siswa_user.DATA_SISWA
            ).order_by("-TANGGAL_PEMINJAMAN")
            return queryset
...
class RiwayatPeminjamanGuruListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if is_in_group(current_user, "Guru"):
            data_guru_user = DataGuruUser.objects.get(USER=current_user)
            queryset = RiwayatPeminjamanGuru.objects.filter(
                DATA_GURU=data_guru_user.DATA_GURU
            )
            return queryset
...
class AccPengajuanPeminjamanGuruView(APIView):
    ...
    def get(self, request, pk, format=None):
        ...
        try:
            ...
            return Response(
                data={"status": "Berhasil menyetujui permintaan peminjaman"},
