...
def generateUserGuru(extra_roles=None):
    ...
    # Set username and password
    username = (
        (data.NAMA_LENGKAP + "_" + str(data.TANGGAL_LAHIR.year))
        .lower()
        .replace(" ", "_")
    )
    password = "merdeka123"
...
def generateUserOrangTua():
    ...
    # Set username and password
    username = (
        (data.NAMA_AYAH + "_" + str(data.TAHUN_LAHIR_AYAH.year))
        .lower()
        .replace(" ", "_")
    )
    password = "merdeka123"
...
def testPostWithFileView(self, url: str, post_data: dict, file_key: str):
    ...
    uploaded_file = SimpleUploadedFile(
        "data_karyawan_user.csv", b"tes abc", content_type="text/plain"
    )
...
def testUpdateView(
    self, url: str, factory_class: DjangoModelFactory, update_data: dict
):
    # Generate new data
    factory_class()

    response = self.client.put(
        reverse(url, kwargs={"pk": 1}), update_data, format="multipart"
    )

    self.assertEqual(response.status_code, status.HTTP_200_OK)


def testUpdateWithFileView(
    self, url: str, factory_class: DjangoModelFactory, update_data: dict, file_key: str
):
    # Generate new data
    factory_class()
...
def testDeleteView(
    self, url: str, factory_class: DjangoModelFactory, factory_length: int
):
    for _ in range(factory_length):
        factory_class()
