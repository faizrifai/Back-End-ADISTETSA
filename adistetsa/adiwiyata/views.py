...
from .serializers import (
    KaderListSerializer,
    SanitasiDrainaseListSerializer,
    TabunganSampahListSerializer,
    PublikasiListSerializer,
    ReuseReduceRecycleListSerializer,
    KonservasiListSerializer,
    JaringanKerjaListSerializer,
    KaryaInovatifListSerializer,
    KegiatanKaderListSerializer,
    PenerapanPRLHListSerializer,
    PenanamanPohonListSerializer,
    PembibitanPohonListSerializer,
    PemeliharaanPohonListSerializer,
    PemeliharaanSampahListSerializer,
)
...
class PenanamanPohonListView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        new_data.update(
            {
                "TOTAL_POHON": PenanamanPohon.objects.all().aggregate(Sum("JUMLAH"))[
                    "JUMLAH__sum"
                ],
            }
        )
...
class TabunganSampahListView(APIView):
    ...
    def get(self, request, format=None):
        ...
        sampah_kering = (
            TabunganSampah.objects.filter(
                TANGGAL__year=filter_tahun, KATEGORI="Kering"
            ).aggregate(Sum("JUMLAH"))["JUMLAH__sum"],
        )
        sampah_basah = (
            TabunganSampah.objects.filter(
                TANGGAL__year=filter_tahun, KATEGORI="Basah"
            ).aggregate(Sum("JUMLAH"))["JUMLAH__sum"],
        )
        total_tabungan = (
            TabunganSampah.objects.filter(TANGGAL__year=filter_tahun).aggregate(
                Sum("JUMLAH")
            )["JUMLAH__sum"],
        )
