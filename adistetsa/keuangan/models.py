...
class Pembayaran(models.Model):
    ...
    PEMBAYARAN_DPSM_INSINDENTAL = models.CharField(
        blank=True, default=0, max_length=1024, validators=[validasi_keuangan]
    )
    PEMBAYARAN_DPSM_RUTIN = models.CharField(
        blank=True, default=0, max_length=1024, validators=[validasi_keuangan]
    )
    BULAN_PEMBAYARAN_DPSM_RUTIN = models.CharField(
        max_length=1024, blank=True, default=""
    )
    BIMBEL = models.CharField(
        blank=True, default=0, max_length=1024, validators=[validasi_keuangan]
    )
    ...
    def clean(self):
        if (
            self.PEMBAYARAN_DPSM_INSINDENTAL == "0"
            and self.PEMBAYARAN_DPSM_RUTIN == "0"
            and self.BIMBEL == "0"
        ):
            raise ValidationError(
                "Harus ada minimal satu jenis pembayaran dipilih antara lain DPSM RUTIN, DPSM INSINDENTAL, BIMBEL, atau NOMINAL SPP + PEMBAYARAN SPP"
            )

        if (
            self.PEMBAYARAN_DPSM_INSINDENTAL != "0"
            and self.PEMBAYARAN_DPSM_RUTIN != "0"
            or self.PEMBAYARAN_DPSM_INSINDENTAL != "0"
            and self.BIMBEL != "0"
        ):
            raise ValidationError(
                "Pembayaran Insindental Tidak Bisa Dilakukan Bersamaan Dengan Pembayaran Lainnya "
            )
