...
from .custom_function import (
    pembagian_jadwal_mengajar,
    pembagian_tugas_guru_bk,
    pembagian_tugas_guru_tik,
    pembagian_tugas_pokok_tendik,
    rekapitulasi_jam_mengajar,
    rincian_tugas_pokok_tendik,
    tugas_tambahan_kepanitiaan,
)
...
class BahanBukuUPM(models.Model):
    ...
    KATEGORI = models.CharField(
        max_length=255, choices=ENUM_KATEGORI_BAHAN_BUKU, default=""
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.GENERATE:
            if self.KATEGORI == "Pembagian Jadwal Mengajar":
                ...
                self.KATEGORI
                == "Pembagian Tugas Pokok dan Tambahan Tenaga Kependidikan"
            ):
                data = PembagianTugasPokokTambahanTendik.objects.all()
                pembagian_tugas_pokok_tendik(self, data)
            elif (
                self.KATEGORI == "Rincian Tugas Pokok dan Tambahan Tenaga Kependidikan"
            ):
                ...
                data = TugasTambahanKepanitiaanTendik.objects.all().order_by(
                    "BIDANG__KODE_BIDANG",
                )
