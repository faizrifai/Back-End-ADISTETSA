...
def pembagian_jadwal_mengajar(self):
    ...
    kelas = OfferingKelas.objects.all().order_by(
        "KELAS__TINGKATAN", "-KELAS__JURUSAN__NAMA", "OFFERING"
    )
    jadwal = (
        JadwalMengajar.objects.all()
        .filter(TAHUN_AJARAN=self.TAHUN_AJARAN, SEMESTER=self.SEMESTER)
        .order_by("GURU")
        .annotate(JUMLAH=Count("WAKTU_PELAJARAN"))
    )
    kelas_siswa = KelasSiswa.objects.all().filter(
        KELAS__KELAS__TAHUN_AJARAN=self.TAHUN_AJARAN
    )
    ...
    for g in guru:
        if not any(
            d["GURU"] == g["GURU"] and d["MATA_PELAJARAN"] == g["MATA_PELAJARAN"]
            for d in jumlah_keseluruhan
        ):
            data_baru = g
            ...
            data_baru = next(
                d
                for d in jumlah_keseluruhan
                if d["GURU"] == g["GURU"] and d["MATA_PELAJARAN"] == g["MATA_PELAJARAN"]
            )

        for data in jadwal:
            if (
                data.GURU.NAMA_LENGKAP == g["GURU"]
                and str(data.MATA_PELAJARAN) == g["MATA_PELAJARAN"]
            ):
    ...
    for data in kelas:
        jumlah_siswa[str(data.KELAS.TINGKATAN)] = kelas_siswa.filter(
            KELAS__KELAS__TINGKATAN=data.KELAS.TINGKATAN
        ).count()
    ...
    df.to_excel(
        writer,
        sheet_name=self.KATEGORI,
        index=True,
    )
    dt.to_excel(
        writer,
        sheet_name="TOTAL_JUMLAH",
        index=False,
    )
    ...
    return ContentFile(
        b.getvalue(), self.KATEGORI + "_" + str(datetime.date.today()) + ".xlsx"
    )


def rekapitulasi_jam_mengajar(self):
    kelas_tingkatan = Kelas.objects.all().order_by(
        "TINGKATAN",
    )
    jadwal = (
        JadwalMengajar.objects.all()
    ...
    df_ = pd.DataFrame(jumlah_keseluruhan).set_index(
        [
            "GURU",
            "NIP",
        ]
...
def pembagian_tugas_guru_bk(self, count):
    ...
    for data in pembagian_tugas:
        ...
        if not (exist):
            guru.append(
                {
                    "NAMA_NIP_JABATAN": data.DATA_GURU.NAMA_LENGKAP,
                    "PANGKAT_GOLONGAN": data.DATA_GURU.PANGKAT
                    + ","
                    + str(cari_pangkat(data.DATA_GURU)),
                }
            )
    ...
    for g in guru:
        if not any(
            d["NAMA_NIP_JABATAN"] == g["NAMA_NIP_JABATAN"] for d in jumlah_keseluruhan
        ):
            data_baru = g
            data_baru = gabung_dictionary(data_baru, jumlah)
        else:
            data_baru = next(
                d
                for d in jumlah_keseluruhan
                if d["NAMA_NIP_JABATAN"] == g["NAMA_NIP_JABATAN"]
            )
        ...
        for data in pembagian_tugas.filter(
            DATA_GURU__NAMA_LENGKAP=g["NAMA_NIP_JABATAN"]
        ):
    ...
    self.FILE = ContentFile(
        b.getvalue(), "PEMBAGIAN_BK" + "_" + str(datetime.date.today()) + ".xlsx"
    )
...
def pembagian_tugas_guru_tik(self, count):
    ...
    writer.save()
    self.FILE = ContentFile(
        b.getvalue(), "PEMBAGIAN_TIK" + "_" + str(datetime.date.today()) + ".xlsx"
    )
...
def pembagian_tugas_pokok_tendik(self, count):
    ...
    for data in pembagian_tugas.filter(TAHUN_AJARAN=self.TAHUN_AJARAN):
        exist = multiple_value_exist({"NAMA": data.DATA_GURU.NAMA_LENGKAP}, guru)
        if not (exist):
            guru.append(
                {
    ...
    writer.save()
    self.FILE = ContentFile(
        b.getvalue(), "PEMBAGIAN_TUGAS" + "_" + str(datetime.date.today()) + ".xlsx"
    )
...
def rincian_tugas_pokok_tendik(self, count):
    ...
    for data in pembagian_tugas.filter(TAHUN_AJARAN=self.TAHUN_AJARAN):
        exist = multiple_value_exist(
            {
                "NAMA_JABATAN": str(data.DATA_GURU.NAMA_LENGKAP)
                + ", "
                + str(data.DATA_GURU.PANGKAT),
            },
            guru,
        )
        if not (exist):
    ...
    for g in guru:
        if not any(d["NAMA_JABATAN"] == g["NAMA_JABATAN"] for d in jumlah_keseluruhan):
            ...
            data_baru = next(
                d for d in jumlah_keseluruhan if d["NAMA_JABATAN"] == g["NAMA_JABATAN"]
            )
        if not any(d["NAMA_JABATAN"] == g["NAMA_JABATAN"] for d in jumlah_keseluruhan):
            jumlah_keseluruhan.append(data_baru)
    df = pd.DataFrame(jumlah_keseluruhan)
    ...
    writer.save()
    self.FILE = ContentFile(
        b.getvalue(), "RINCIAN_TUGAS" + "_" + str(datetime.date.today()) + ".xlsx"
    )
...
def tugas_tambahan_kepanitiaan(self, count):
    ...
    df.to_excel(
        writer,
        sheet_name="TUGAS_TAMBAHAN_KEPANITIAAN",
        index=True,
    )
    ...
    self.FILE = ContentFile(
        b.getvalue(),
        "TUGAS_TAMBAHAN_KEPANITIAAN" + "_" + str(datetime.date.today()) + ".xlsx",
    )
...
def buat_file_prototype(self):
        ...
        for g in guru:
            if not any(
                ...
                data_baru = next(
                    ...
                    for d in jumlah_keseluruhan
                    if d["GURU"] == g["GURU"]
                    and d["MATA_PELAJARAN"] == g["MATA_PELAJARAN"]
                )
    ...
    # Pembagian Tugas BK
        ...
        pembagian_tugas = OfferingKelas.objects.all().order_by(
            "KELAS__TINGKATAN",
        )

        kelas_tingkatan = OfferingKelas.objects.all().order_by(
            "KELAS__TINGKATAN",
        )
        kelas_siswa = KelasSiswa.objects.all().filter(
            KELAS__KELAS__TAHUN_AJARAN=self.TAHUN_AJARAN
        ...
        for data in pembagian_tugas:
            exist = multiple_value_exist(
                {"NAMA_NIP": data.DATA_GURU.NAMA_LENGKAP}, guru
            )

            if not (exist):
                guru.append({"NAMA_NIP": data.DATA_GURU.NAMA_LENGKAP})
        ...
        pembagian_tugas_tendik = OfferingKelas.objects.all().order_by(
            "KELAS__TINGKATAN",
        )
        ...
        for g in guru:
            if not any(d["NAMA"] == g["NAMA"] for d in jumlah_keseluruhan):
                ...
                data_baru = next(
                    d for d in jumlah_keseluruhan if d["NAMA"] == g["NAMA"]
                )
