...
class KalenderPendidikan(models.Model):
    ...
    def clean(self):
        kalender_obj = KalenderPendidikan.objects.filter(TANGGAL_MULAI=self.TANGGAL_MULAI).exclude(pk=self.pk)
    ...
    def get_url(self, day):
        ...
        return u'<a href="%s" style="position: relative; display: block;">%s</a>' % (url, str(day))