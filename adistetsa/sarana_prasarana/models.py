...
def cek_jangka_pendek(data):
    # cek pengajuan
    obj_pengajuan = PengajuanPeminjamanRuangan.objects.filter(
    ...
    for diajukan in obj_pengajuan:
        if diajukan.JENIS_PEMINJAMAN == "Jangka Pendek":
            ...
            if cek_konflik(
                diajukan.TANGGAL_PEMAKAIAN,
                data.TANGGAL_PEMAKAIAN,
                diajukan.TANGGAL_BERAKHIR,
            ) or cek_konflik(
                diajukan.TANGGAL_PEMAKAIAN,
                ...
                if cek_konflik(
                    diajukan.JAM_PENGGUNAAN, data.JAM_PENGGUNAAN, diajukan.JAM_BERAKHIR
                ) or cek_konflik(
                    diajukan.JAM_PENGGUNAAN, data.JAM_BERAKHIR, diajukan.JAM_BERAKHIR
                ):
                    sukses = False
                else:
                    sukses = True
    ...
    if not sukses:
        raise ValidationError(
            "Ruangan sudah diajukan pada tanggal yang sama dan jam pemakaian bersinggungan dengan waktu yang dipilih"
        )

    # cek riwayat
    obj_riwayat = RiwayatPeminjamanRuangan.objects.filter(RUANGAN=data.RUANGAN).filter(
    ...
    for riwayat in obj_riwayat:
        if riwayat.JENIS_PEMINJAMAN == "Jangka Pendek":
            ...
            if cek_konflik(
                riwayat.TANGGAL_PEMAKAIAN,
                data.TANGGAL_PEMAKAIAN,
                riwayat.TANGGAL_BERAKHIR,
            ) or cek_konflik(
                riwayat.TANGGAL_PEMAKAIAN,
                data.TANGGAL_BERAKHIR,
                riwayat.TANGGAL_BERAKHIR,
                ...
                # Menghindari konflik jam
                if cek_konflik(
                    riwayat.JAM_PENGGUNAAN, data.JAM_PENGGUNAAN, riwayat.JAM_BERAKHIR
                ) or cek_konflik(
                    riwayat.JAM_PENGGUNAAN, data.JAM_BERAKHIR, riwayat.JAM_BERAKHIR
                ):
                    sukses = False
                else:
                    sukses = True
            ...
            # cek hari pemakaian konflik
            if cek_hari_konflik(
                riwayat.TANGGAL_PEMAKAIAN, data.TANGGAL_PEMAKAIAN, data.TANGGAL_BERAKHIR
            ):
                # Menghindari konflik jam
                if cek_konflik(
                    riwayat.JAM_PENGGUNAAN, data.JAM_PENGGUNAAN, riwayat.JAM_BERAKHIR
    ...
    if not sukses:
        raise ValidationError(
            "Ruangan sudah dipinjam pada tanggal yang sama dan jam pemakaian bersinggungan dengan waktu yang dipilih"
        )
...
def cek_jangka_panjang(data):
    ...
    if not sukses:
        raise ValidationError(
            "Ruangan sudah diajukan pada hari yang sama dan jam pemakaian bersinggungan dengan waktu yang dipilih"
        )
    ...
    if not sukses:
        raise ValidationError(
            "Ruangan sudah dipinjam pada hari yang sama dan jam pemakaian bersinggungan dengan waktu yang dipilih"
        )
...
class JenisSarana(models.Model):
    ...
    KATEGORI = models.CharField(
        max_length=255, validators=[cek_huruf_besar_awal_kalimat]
    )
...
class PengajuanPeminjamanRuangan(models.Model):
    ...
    def clean(self):
        if self.TANGGAL_PEMAKAIAN > self.TANGGAL_BERAKHIR:
            raise ValidationError(
                "Tanggal Pemakaian tidak boleh lebih dari Tanggal Berakhir"
            )
    ...
    if instance.STATUS == "Sedang Dipinjam":
        try:
            ...
            if instance.TANDA_TANGAN:
                obj.TANDA_TANGAN = duplikat_file(
                    instance, instance.TANDA_TANGAN.read(), instance.TANDA_TANGAN.name
                )
...
post_save.connect(
    post_save_pengajuan_peminjaman_ruangan, sender=PengajuanPeminjamanRuangan
)
...
class PengajuanPeminjamanBarang(models.Model):
    ...
    TANDA_TANGAN = models.FileField(
        max_length=255, upload_to="PeminjamanBarang", blank=True
    )
...
class RiwayatPeminjamanBarang(models.Model):
    ...
    STATUS_PEMINJAMAN = models.CharField(
        max_length=255, choices=ENUM_STATUS_PEMINJAMAN, default="Sedang Dipinjam"
    )
    TANDA_TANGAN = models.FileField(
        max_length=255, upload_to="PeminjamanBarang", blank=True
    )
        ...
        try:
            for data in instance.ALAT.values():
                ...
                if instance.TANDA_TANGAN:
                    obj.TANDA_TANGAN = duplikat_file(
                        instance,
                        instance.TANDA_TANGAN.read(),
                        instance.TANDA_TANGAN.name,
                    )
...
post_save.connect(
    post_save_pengajuan_peminjaman_barang, sender=PengajuanPeminjamanBarang
)
