...
class PengajuanPeminjamanRuanganFactory(CleanModelFactory):
    ...
    TANGGAL_BERAKHIR = factory.LazyAttribute(
        lambda self: self.TANGGAL_PEMAKAIAN + datetime.timedelta(days=30)
    )
    JAM_PENGGUNAAN = factory.SelfAttribute("TANGGAL_PENGAJUAN")
    JAM_BERAKHIR = factory.LazyAttribute(
        lambda self: self.JAM_PENGGUNAAN + datetime.timedelta(hours=2)
    )
    JENIS_PEMINJAMAN = factory.Faker(
        "random_element", elements=random_enum(ENUM_JENIS_PEMINJAMAN)
    )
...
class PengajuanPeminjamanBarangFactory(CleanModelFactory):
    ...
    TANGGAL_PENGEMBALIAN = factory.LazyAttribute(
        lambda self: self.TANGGAL_PENGGUNAAN + datetime.timedelta(days=30)
    )
    KETERANGAN = factory.Faker("sentence")
    TANDA_TANGAN = factory.django.ImageField(color="white")
