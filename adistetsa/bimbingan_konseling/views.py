...
class DaftarKonsultasiListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if query_params.get("search"):
            data_siswa_user = DataSiswaUser.objects.filter(
                DATA_SISWA__NAMA__icontains=query_params.get("search")
            )
            data_guru_user = DataGuruUser.objects.filter(
                DATA_GURU__NAMA_LENGKAP__icontains=query_params.get("search")
            )
            data_karyawan_user = DataKaryawanUser.objects.filter(
                DATA_KARYAWAN__NAMA_LENGKAP__icontains=query_params.get("search")
            )
        ...
        return (
            super()
            .get_queryset()
            .filter(KONSELOR__USER=current_user)
            .order_by("-TANGGAL_KONSULTASI")
        )
    ...
    def get_queryset(self):
        ...
        if is_in_group(current_user, "Siswa"):
            ...
            kelas_siswa = KelasSiswa.objects.filter(
                NIS=data_siswa, KELAS__KELAS__TINGKATAN="X"
            ).first()
            try:
                queryset = PeminatanLintasMinat.objects.get(
                    KELAS_SISWA=kelas_siswa, KATEGORI="Angket Peminatan"
                )
            except PeminatanLintasMinat.DoesNotExist:
                queryset = PeminatanLintasMinat.objects.create(
                    KELAS_SISWA=kelas_siswa, KATEGORI="Angket Peminatan"
                )
...
class AngketLintasMinatSiswaDetailView(APIView):
    ...
    def get_queryset(self):
        ...
        if is_in_group(current_user, "Siswa"):
            ...
            try:
                queryset = PeminatanLintasMinat.objects.get(
                    KELAS_SISWA=kelas_siswa, KATEGORI="Angket Lintas Minat"
                )
            except PeminatanLintasMinat.DoesNotExist:
                queryset = PeminatanLintasMinat.objects.create(
                    KELAS_SISWA=kelas_siswa, KATEGORI="Angket Lintas Minat"
...
class AngketDataDiriSiswaDetailView(APIView):
    ...
    def get_queryset(self):
        ...
        if is_in_group(current_user, "Siswa"):
            ...
            try:
                queryset = PeminatanLintasMinat.objects.get(
                    KELAS_SISWA=kelas_siswa, KATEGORI="Angket Data Diri"
                )
            except PeminatanLintasMinat.DoesNotExist:
                queryset = PeminatanLintasMinat.objects.create(
                    KELAS_SISWA=kelas_siswa, KATEGORI="Angket Data Diri"
