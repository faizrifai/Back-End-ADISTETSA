...
class KTSPDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def update(self, request, *args, **kwargs):
        try:
            ...
            if "UNIQUE" in str(e):
                return Response(
                    data={"error": "Data dengan tahun ajaran yang dipilih sudah ada"},
...
class JadwalMengajarGuruListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        data_guru_user = apps.get_model(
            "kustom_autentikasi", "DataGuruUser"
        ).objects.get(USER=current_user)
        queryset = JadwalMengajar.objects.filter(
...
class JurnalBelajarGuruListView(generics.ListCreateAPIView):
    ...
    permission_classes = [HasGroupPermissionAny]
    required_groups = {"GET": ["Guru"], "POST": ["Guru"]}
...
class AbsensiSiswaDetailView(generics.RetrieveUpdateAPIView):
    ...
    permission_classes = [HasGroupPermissionAny]
    required_groups = {"GET": ["Guru"], "PATCH": ["Guru"]}
