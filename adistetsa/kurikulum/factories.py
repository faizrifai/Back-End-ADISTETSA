...
class KelasFactory(DjangoModelFactory):
    class Meta:
        model = Kelas
        django_get_or_create = ("TAHUN_AJARAN", "TINGKATAN", "JURUSAN")
...
class OfferingKelasFactory(DjangoModelFactory):
    class Meta:
        model = OfferingKelas
        django_get_or_create = ("KELAS", "OFFERING")
...
class KelasSiswaFactory(DjangoModelFactory):
    class Meta:
        model = KelasSiswa
        django_get_or_create = ("NIS", "KELAS")
...
class JadwalMengajarFactory(DjangoModelFactory):
    class Meta:
        ...
        django_get_or_create = ("KELAS", "MATA_PELAJARAN", "HARI")
...
class NilaiRaportFactory(DjangoModelFactory):
    class Meta:
        model = NilaiRaport
        django_get_or_create = ("RAPORT", "MATA_PELAJARAN")

    RAPORT = models.ForeignKey(Raport, on_delete=models.CASCADE)
    MATA_PELAJARAN = factory.Iterator(MataPelajaran.objects.all())
    KELOMPOK_MATA_PELAJARAN = factory.Faker(
        "random_element", elements=random_enum(ENUM_KELOMPOK_MATA_PELAJARAN)
    )
...
class KTSPFactory(DjangoModelFactory):
    class Meta:
        model = KTSP
        django_get_or_create = ("TAHUN_AJARAN",)
