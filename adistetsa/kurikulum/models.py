...
class TahunAjaran(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["TAHUN_AJARAN_AWAL", "TAHUN_AJARAN_AKHIR"],
...
class Kelas(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["TAHUN_AJARAN", "TINGKATAN", "JURUSAN"],
                name="%(app_label)s_%(class)s_unique",
            )
        ]
    ...
    def save(self, *args, **kwargs):
        self.KODE_KELAS = (
            self.TINGKATAN + " " + self.JURUSAN.NAMA + "-" + str(self.TAHUN_AJARAN)
        )
...
class KTSP(models.Model):
    ...
    def __str__(self):
        return (
            self.NAMA_FILE.name
            + " - "
            + str(self.TAHUN_AJARAN.TAHUN_AJARAN_AWAL)
            + "/"
            + str(self.TAHUN_AJARAN.TAHUN_AJARAN_AKHIR)
        )
...
class SilabusRPB(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["MATA_PELAJARAN", "TAHUN_AJARAN", "KELAS", "SEMESTER"],
                name="%(app_label)s_%(class)s_unique",
            )
        ]
...
class KelasSiswa(models.Model):
    ...
    def clean(self):
        kelas = KelasSiswa.objects.filter(
            NIS=self.NIS, KELAS__KELAS__TINGKATAN=self.KELAS.KELAS.TINGKATAN
        ).exclude(ID=self.ID)

        if kelas:
            for data in kelas:
                if (
                    data.KELAS.KELAS.TINGKATAN == self.KELAS.KELAS.TINGKATAN
                    and data.KELAS.KELAS.TAHUN_AJARAN == self.KELAS.KELAS.TAHUN_AJARAN
                ):
                    raise ValidationError(
                        {
                            "KELAS": self.NIS.NAMA
                            + " sudah terdaftar pada tingkatan dan tahun ajaran yang sama"
                        }
                    )
...
class DaftarJurnalBelajar(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["GURU", "MATA_PELAJARAN", "KELAS", "SEMESTER"],
                name="%(app_label)s_%(class)s_unique",
            )
        ]
    ...
    def __str__(self):
        return (
            self.MATA_PELAJARAN.NAMA + " " + str(self.KELAS) + " " + self.SEMESTER.NAMA
        )
...
class JurnalBelajar(models.Model):
    ...
    def __str__(self):
        return (
            str(self.DAFTAR.MATA_PELAJARAN)
            + " - "
            + str(self.DAFTAR.KELAS)
            + " - "
...
class JadwalPekanEfektifSemester(models.Model):
    ...
    def __str__(self):
        return (
            self.BULAN
            + " - Jumlah Minggu = "
            + str(self.JUMLAH_MINGGU)
            + " || Jumlah Minggu Efektif = "
            + str(self.JUMLAH_MINGGU_EFEKTIF)
        )
...
class Raport(models.Model):
    ...
    SEMESTER = models.ForeignKey(DataSemester, on_delete=models.CASCADE)
    BUKU_INDUK = models.ForeignKey("tata_usaha.BukuInduk", on_delete=models.CASCADE)

    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["KELAS_SISWA", "SEMESTER"],
                name="%(app_label)s_%(class)s_unique",
            )
        ]
