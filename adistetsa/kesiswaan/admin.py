...
class KatalogEkskulAdmin(RootSubAdmin, ImportExportModelAdmin):
    ...
    def aksi(self, obj):
        ...
        return mark_safe(
            '<a href="%s%d/anggotaekskul">%s</a>' % (base_url, obj.ID, "Buka Anggota")
        )
...
class JurnalEkskulAdmin(BaseSubAdminExport):
    ...
    def absensi(self, obj):
        ...
        return mark_safe(
            '<a href="%s%d/jurnalekskul/%d/absensiekskul">%s</a>'
...
class DaftarJurnalEkskulAdmin(RootSubAdmin):
    ...
    def aksi(self, obj):
        ...
        return mark_safe(
            '<a href="%s%d/jurnalekskul">%s</a>' % (base_url, obj.ID, "Buka Jurnal")
        )
...
class NilaiEkskulAdmin(BaseSubAdminExport):
    model = NilaiEkskul
    search_fields = (
        "DATA_ANGGOTA__KELAS_SISWA__NIS__NAMA",
        "DATA_ANGGOTA__EKSKUL__NAMA",
    )
    list_display = [
        "DATA_ANGGOTA",
        "ekskul",
...
class NilaiEkskulPelatihAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        if is_in_group(request.user, "Pelatih"):
            ...
            anggota = AnggotaEkskul.objects.filter(
                TAHUN_AJARAN__ID__in=tahun_ajaran, EKSKUL__ID__in=ekskul
            ).values("ID")
