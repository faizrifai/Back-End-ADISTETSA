...
class JurnalEkskulSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        id_daftar_jurnal_ekskul = request.parser_context.get('kwargs').get('id_daftar_jurnal_ekskul')

        validated_data['PELATIH'] = data_pelatih_user.DATA_PELATIH
        validated_data['DAFTAR'] = DaftarJurnalEkskul.objects.get(pk=id_daftar_jurnal_ekskul)
...
class PengajuanEkskulSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        try:
            mengajukan = PengajuanEkskul.objects.get(KELAS_SISWA=kelas_siswa, EKSKUL=ekskul, TAHUN_AJARAN=tahun_ajaran_aktif)
        except:
            mengajukan = None

        if (mengajukan):
                raise serializers.ValidationError('Sudah pernah mengajukan pendaftaran, silahkan tunggu konfirmasi dari pelatih')

        # cek sudah terdaftar
        try:
            terdaftar = AnggotaEkskul.objects.get(KELAS_SISWA=kelas_siswa, EKSKUL=ekskul, TAHUN_AJARAN=tahun_ajaran_aktif)
        except:
            terdaftar = None

        if (terdaftar):
                raise serializers.ValidationError('Sudah terdaftar di ekskul ' + ekskul.NAMA)
