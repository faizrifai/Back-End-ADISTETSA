...
class PengajuanLaporanPelanggaran(models.Model):
    ...
    STATUS_PENGAJUAN = models.CharField(
        max_length=255, choices=ENUM_PENGAJUAN_PELANGGARAN, default="Diajukan"
    )
        ...
        if instance.STATUS_PENGAJUAN == "Disetujui":
            ...
            try:
                riwayat = RiwayatLaporanPelanggaran.objects.create(
                    DATA_SISWA=instance.DATA_SISWA,
                    BUKTI_PELANGGARAN=duplikat_file(
                        instance,
                        instance.BUKTI_PELANGGARAN.read(),
                        instance.BUKTI_PELANGGARAN.name,
                    ),
...
class PengajuanProgramKebaikan(models.Model):
    ID = models.BigAutoField(primary_key=True)
    DATA_SISWA = models.ForeignKey(DataSiswa, on_delete=models.CASCADE)
    BUKTI_PROGRAM_KEBAIKAN = models.FileField(
        max_length=255, upload_to="BuktiProgramKebaikan"
    )
    JENIS_PROGRAM_KEBAIKAN = models.ForeignKey(
        PoinProgramKebaikan, on_delete=models.CASCADE
    )
    TANGGAL_PENGAJUAN = models.DateTimeField(default=timezone.now)
    STATUS_PENGAJUAN = models.CharField(
        max_length=255,
        ...
        if instance.STATUS_PENGAJUAN == "Disetujui":
            ...
            try:
                riwayat = RiwayatProgramKebaikan.objects.create(
                    DATA_SISWA=instance.DATA_SISWA,
                    BUKTI_PROGRAM_KEBAIKAN=duplikat_file(
                        instance,
                        instance.BUKTI_PROGRAM_KEBAIKAN.read(),
                        instance.BUKTI_PROGRAM_KEBAIKAN.name,
                    ),
...
class KatalogEkskul(models.Model):
    ...
    DOKUMENTASI = models.ImageField(
        upload_to="KatalogEkskul", max_length=255, blank=True
    )
...
class JurnalEkskul(models.Model):
    ...
    def __str__(self):
        return (
            str(self.DAFTAR.EKSKUL)
            + " - "
            + str(self.DAFTAR.SEMESTER)
            + " - Pertemuan : "
            + self.PERTEMUAN
        )
...
def post_save_jurnal_ekskul(sender, instance, created, **kwargs):
    try:
        ...
        for kelas_siswa in ekskul:
            anggota_ekskul = AnggotaEkskul.objects.get(
                KELAS_SISWA=kelas_siswa.KELAS_SISWA, EKSKUL=instance.DAFTAR.EKSKUL
            )
            if anggota_ekskul.STATUS == "Nonaktif":
                continue

            AbsensiEkskul.objects.update_or_create(
                NIS=kelas_siswa.KELAS_SISWA.NIS, JURNAL_EKSKUL=instance
            )
...
class AbsensiEkskul(models.Model):
    ...
    KETERANGAN = models.CharField(
        max_length=255, choices=ENUM_KETERANGAN_ABSEN, default="Hadir"
    )
    FILE_KETERANGAN = models.FileField(
        max_length=255, upload_to="AbsensiEkskul", blank=True
    )
...
class AnggotaEkskul(models.Model):
    ...
    def __str__(self):
        return (
            str(self.KELAS_SISWA.NIS.NAMA)
            + " - "
            + str(self.EKSKUL.NAMA)
            + " - "
            + str(self.TAHUN_AJARAN)
        )
...
class ProgramKerjaEkskul(models.Model):
    ...
    EKSKUL = models.ForeignKey(KatalogEkskul, on_delete=models.CASCADE)
    TAHUN_AJARAN = models.ForeignKey(TahunAjaran, on_delete=models.CASCADE)
    FILE_PROGRAM_KERJA = models.FileField(
        max_length=255, upload_to="ProgramKerjaEkskul"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["PELATIH", "EKSKUL", "TAHUN_AJARAN"],
                name="%(app_label)s_%(class)s_unique",
            )
        ]
...
def post_save_raport(sender, instance, created, **kwargs):
    try:
        anggota_ekskul = AnggotaEkskul.objects.filter(
            KELAS_SISWA=instance.KELAS_SISWA, STATUS="Aktif"
        )
        for siswa in anggota_ekskul:
            NilaiEkskul.objects.get_or_create(
                DATA_ANGGOTA=siswa, RAPORT=instance, SEMESTER=instance.SEMESTER
            )
    except Exception as e:
        print(str(e))
