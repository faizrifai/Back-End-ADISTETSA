...
class PengajuanLaporanPelanggaranFactory(DjangoModelFactory):
    ...
    TANGGAL_PENGAJUAN = factory.Faker('date_time', tzinfo=timezone.get_current_timezone())
...
class JadwalEkskulFactory(DjangoModelFactory):
    class Meta:
        model = JadwalEkskul
        django_get_or_create = ('PELATIH','HARI','TAHUN_AJARAN','WAKTU_MULAI','WAKTU_BERAKHIR')
    ...
    WAKTU_BERAKHIR = factory.LazyAttribute(lambda self: self.WAKTU_MULAI + datetime.timedelta(hours = 2))
...
class PengajuanEkskulFactory(DjangoModelFactory):
    ...
    EKSKUL = factory.Iterator(KatalogEkskul.objects.all())
    TAHUN_AJARAN = factory.LazyAttribute(lambda self: self.KELAS_SISWA.KELAS.KELAS.TAHUN_AJARAN)
    TANGGAL_PENGAJUAN = factory.Faker('date_time', tzinfo=timezone.get_current_timezone())