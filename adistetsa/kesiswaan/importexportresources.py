...
class AnggotaEkskulResource(resources.ModelResource):
    kelas_siswa = Field(
        column_name="KELAS_SISWA", attribute="KELAS_SISWA", widget=CharWidget()
    )

    tahun_ajaran = Field(
        column_name="TAHUN_AJARAN", attribute="TAHUN_AJARAN", widget=CharWidget()
    )
    ekskul = Field(
        column_name="EKSKUL",
        attribute="EKSKUL",
...
class JurnalEkskulResource(resources.ModelResource):
    ...
    daftar = Field(column_name="DAFTAR", attribute="DAFTAR", widget=CharWidget())
...
class JadwalEkskulResource(resources.ModelResource):
    ...
    def before_import_row(self, row, **kwargs):
        ...
        try:
            tahun_ajaran_aktif = TahunAjaran.objects.get(
                TAHUN_AJARAN_AWAL=tahun_ajaran[0], TAHUN_AJARAN_AKHIR=tahun_ajaran[1]
            )
...
class PelanggaranSiswaResource(resources.ModelResource):
    data_siswa = Field(
        column_name="DATA_SISWA", attribute="DATA_SISWA", widget=CharWidget()
    )
...
class RiwayatProgramKebaikanResource(resources.ModelResource):
    ...
    class Meta:
        ...
        fields = (
            "data_siswa",
            "jenis_program_kebaikan",
            "TANGGAL_PENGAJUAN",
            "BUKTI_PROGRAM_KEBAIKAN",
            "STATUS_PENGAJUAN",
        )
        exclude = ("ID",)
